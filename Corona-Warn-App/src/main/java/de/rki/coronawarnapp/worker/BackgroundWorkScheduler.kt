...
object BackgroundWorkScheduler {
    ...
    private fun Operation.logOperationSchedule(workType: WorkType) = this.result.addListener({
        if (BuildConfig.DEBUG) Timber.d("%s completed.", workType.uniqueName)
    }, { it.run() }).also { if (BuildConfig.DEBUG) Timber.d("%s scheduled.", workType.uniqueName) }

    /**
     * Log operation cancellation
     */
    private fun Operation.logOperationCancelByTag(workTag: WorkTag) = this.result.addListener({
        if (BuildConfig.DEBUG) Timber.d("All work with tag %s canceled.", workTag.tag)
    }, { it.run() })
        .also { if (BuildConfig.DEBUG) Timber.d("Canceling all work with tag %s", workTag.tag) }

    /**
     * Log work active status
     */
    private fun logWorkActiveStatus(tag: String, active: Boolean) {
        if (BuildConfig.DEBUG) Timber.d("Work type %s is active: %b", tag, active)
    }
}
